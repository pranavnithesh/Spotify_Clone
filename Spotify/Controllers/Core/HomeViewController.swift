...
enum BrowseSectionType {
    ...
    case recommendedTracks(viewModels: [RecommendedTrackCellViewMode])
...
class HomeViewController: UIViewController {
    ...
    public var collectionView: UICollectionView = UICollectionView(frame: .zero, collectionViewLayout: UICollectionViewCompositionalLayout { (sectionIndex, _) -> NSCollectionLayoutSection? in
    ...
    private func fetchdata() {
        ...
        group.notify(queue: .main) {
            guard let newAlbums = newRelease?.albums.items, let playlists = featuredPlaylist?.playlists.items, let tracks = recommendations?.tracks else {
                return
            }
    ...
    private func configureModels(newAlbums: [Album], playlists: [Playlist], tracks: [AudioTrack]) {
        ...
        sections.append(.recommendedTracks(viewModels: tracks.compactMap({
            return RecommendedTrackCellViewMode(name: $0.name, artworkURL: URL(string: $0.album? .images.first?.url ?? ""), artistName:$0.artists.first?.name ?? "-")
        })))
