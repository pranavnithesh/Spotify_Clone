...
class PlaylistViewController: UIViewController {
    ...
    private var viewModels = [RecommendedTrackCellViewMode]()
            ...
            DispatchQueue.main.async {
                ...
                case .success(let model):
                    ...
                    self?.viewModels = model.tracks.items.compactMap({
                        RecommendedTrackCellViewMode(name: $0.track.name, artworkURL: URL(string: $0.track.album?.images.first?.url ?? ""), artistName: $0.track.artists.first?.name ?? "-")
                    })
    ...
    @objc private func didLongPress(_ gesture: UILongPressGestureRecognizer) {
        guard gesture.state == .began, self.isOwner else {
            return
        }
        ...
        let actionSheet = UIAlertController(title: trackToDelete.name, message: "Would you like to remove this from your playlist?", preferredStyle: .actionSheet)
